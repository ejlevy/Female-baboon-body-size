...
## This script recreates the analyses for 
  ...
  # Emily J. Levy, Anna Lee, I. Long'ida Siodi, Emma C. Helmich, Elise J. Malone, Emily M. McLean, Maggie J. Pickard, Riddhi Ranjithkumar, Jenny Tung, Elizabeth A. Archie, Susan C. Alberts 
...
# Make sure ID and momID are categorical variables
sr$ID <- as.factor(sr$ID)
leg$ID <- as.factor(leg$ID)
forearm$ID <- as.factor(forearm$ID)
sr$momID <- as.factor(sr$momID)
leg$momID <- as.factor(leg$momID)
forearm$momID <- as.factor(forearm$momID)
# Pedigree
...
# Piecewise linear-linear with asreml (+ Figure S3)-------------------------------
...
find_knot_linear_leg <- function(knot_input){
  ...
  fit <- asreml(fixed = leg_cm ~ age + timespline_var + leg_rating, 
...
linear_leg_ped <- asreml(fixed = leg_cm ~ age + timespline + leg_rating, 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = leg)
...
ggplot(leg, aes(x=age, y=leg_cm)) + geom_point(alpha = 0.2) + 
...
find_knot_linear_forearm <- function(knot_input){
  ...
  fit <- asreml(fixed = forearm_cm ~ age + timespline_var, 
...
linear_forearm_ped <- asreml(fixed = forearm_cm ~ age + timespline, 
                             random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = forearm)
...
ggplot(forearm, aes(x=age, y=forearm_cm)) + geom_point(alpha = 0.2) + 
...
# Quadratic threshold with asreml (+ Figure S3) -------------------------------
...
find_knot_quad_leg <- function(knot_input){
  ...
  fit <- asreml(fixed = leg_cm ~ timespline2 + I(timespline2^2) + leg_rating, 
                random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = leg) 
  save <- c(knot_input, summary(fit)$aic[1]) # Save the knot_input and the AIC value of the model
}
...
quad_leg_ped <- asreml(fixed = leg_cm ~ timespline2 + I(timespline2^2) + leg_rating, 
                       random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = leg)
...
find_knot_quad_forearm <- function(knot_input){
  ...
  fit <- asreml(fixed = forearm_cm ~ timespline + I(timespline^2), 
                random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = forearm) 
  save <- c(knot_input, summary(fit)$aic[1]) # Save the knot_input and the AIC value of the model
}
...
quad_forearm_ped <- asreml(fixed = forearm_cm ~ timespline2 + I(timespline2^2), 
                           random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = forearm)
...
### Leg 
loglog_leg <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating, 
                     random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = leg)
...
### Forearm 
loglog_forearm <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2), 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, data = forearm)
...
# Performance package. (EJL note: I don't think perfornace::r2 will work on all versions of ASReml-R!)
conditional_sr <- c("Shoulder-rump", "Conditional R2", unlist(r2(linear_sr))[1], unlist(r2(quad_sr))[1], unlist(r2(loglog_sr))[1])
marginal_sr <- c("Shoulder-rump", "Marginal R2", unlist(r2(linear_sr))[2], unlist(r2(quad_sr))[2], unlist(r2(loglog_sr))[2])
conditional_leg <- c("Leg", "Conditional R2", unlist(r2(linear_leg))[1], unlist(r2(quad_leg))[1], unlist(r2(loglog_leg))[1])
marginal_leg <- c("Leg", "Marginal R2", unlist(r2(linear_leg))[2], unlist(r2(quad_leg))[2], unlist(r2(loglog_leg))[2])
conditional_forearm <- c("Forearm", "Conditional R2", unlist(r2(linear_forearm))[1], unlist(r2(quad_forearm))[1], unlist(r2(loglog_forearm))[1])
marginal_forearm <- c("Forearm", "Marginal R2", unlist(r2(linear_forearm))[2], unlist(r2(quad_forearm))[2], unlist(r2(loglog_forearm))[2])

r2 <- data.frame(rbind(conditional_sr, marginal_sr, conditional_leg, marginal_leg, conditional_forearm, marginal_forearm))
colnames(r2) <- c("Body part", "Output", "Piecewise linear-linear", "Piecewise quadratic threshold", "Log-log quadratic", "Cube root")
...
# Shoulder-rump 
rain_1year_sr <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + one_year_rain + mom_proprank_birth + factor(maternal_loss_0isNo), 
                        random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                        data = subset(sr, !is.na(one_year_rain) & !is.na(maternal_loss_0isNo)))
wald(rain_1year_sr)
...
# Leg 
rain_1year_leg <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + one_year_rain + leg_rating + mom_proprank_birth + factor(maternal_loss_0isNo), 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                         data = subset(leg, !is.na(one_year_rain) & !is.na(maternal_loss_0isNo)))
# Warning message:
#   In asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + one_year_rain +  :
#               Some components changed by more than 1% on the last iteration.
# Same if I do 10k iterations
wald(rain_1year_leg)
...
# Arm 
rain_1year_arm <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + one_year_rain + mom_proprank_birth + factor(maternal_loss_0isNo), 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                         data = subset(forearm, !is.na(one_year_rain) & !is.na(maternal_loss_0isNo)))
wald(rain_1year_arm)
...
rain1_p$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
rain1_beta$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
# Models: Cumulative Rainfall 4 years + Maternal loss --------------------------------------

# Shoulder rump 
rain_4year_sr <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + four_years_rain + mom_proprank_birth + factor(maternal_loss_0isNo), 
                        random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                        data = subset(sr, !is.na(four_years_rain)  & !is.na(maternal_loss_0isNo)))
wald(rain_4year_sr)
...
# Leg 
rain_4year_leg <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + four_years_rain + leg_rating + mom_proprank_birth + factor(maternal_loss_0isNo), 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                         data = subset(leg, !is.na(four_years_rain)  & !is.na(maternal_loss_0isNo)))
wald(rain_4year_leg)
...
# Arm 
rain_4year_arm <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + four_years_rain + mom_proprank_birth + factor(maternal_loss_0isNo), 
                         random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                         data = subset(forearm,!is.na(four_years_rain)  & !is.na(maternal_loss_0isNo)))
wald(rain_4year_arm)
...
rain4_p$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
rain4_beta$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
# Models: Proportion of drought days 1 year + Maternal loss --------------------------------------
# Shoulder rump 
droughtday_1year_sr <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + prop_droughtdays_1year + mom_proprank_birth + factor(maternal_loss_0isNo), 
                              random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                              data = subset(sr, !is.na(prop_droughtdays_1year) & !is.na(maternal_loss_0isNo)))
...
# Leg 
droughtday_1year_leg <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + prop_droughtdays_1year + leg_rating + mom_proprank_birth + factor(maternal_loss_0isNo), 
                               random = ~ID + vm(ID, inverse_pedigree) + factor(momID), maxiter = 400, 
                               data = subset(leg, !is.na(prop_droughtdays_1year) & !is.na(maternal_loss_0isNo)))
# Warning message:
#   In asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + prop_droughtdays_1year +  :
#               Some components changed by more than 1% on the last iteration.
...
# Arm 
droughtday_1year_arm <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + prop_droughtdays_1year + mom_proprank_birth + factor(maternal_loss_0isNo), 
                               random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                               data = subset(forearm, !is.na(prop_droughtdays_1year) & !is.na(maternal_loss_0isNo)))
...
droughtday1_p$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
droughtday1_beta$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
# Models: Proportion drought days 4 years + Maternal loss  --------------------------------------

# Shoulder rump 
droughtday_4year_sr <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                              random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                              data = subset(sr, !is.na(prop_droughtdays_4year)  & !is.na(maternal_loss_0isNo)))
...
# Leg
droughtday_4year_leg <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                               random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                               data = subset(leg, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
...
# Arm 
droughtday_4year_arm <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                               random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                               data = subset(forearm, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
# Warning message:
#   In asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + prop_droughtdays_1year +  :
#               Some components changed by more than 1% on the last iteration.
...
droughtday4_p$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
droughtday4_beta$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
## Running the 3 drought-days-4-years + maternal loss models
...
# Shoulder rump: 
droughtday_4year_sr_hybrid <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + anubis_admix + I(anubis_admix^2) + factor(maternal_loss_0isNo), 
                                     random = ~ID + momID, maxiter = 400, 
                                     data = subset(data_sr_hybrid, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
...
droughtday_4year_sr_nohybrid <- asreml(fixed = log(sr_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                                       random = ~ID + momID, maxiter = 400,
                                       data = subset(data_sr_hybrid, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
summary(droughtday_4year_sr_nohybrid)$aic - summary(droughtday_4year_sr_hybrid)$aic   

# Leg
droughtday_4year_leg_hybrid <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth + prop_droughtdays_4year + anubis_admix + I(anubis_admix^2) + factor(maternal_loss_0isNo), 
                                      random = ~ID + momID, maxiter = 400, 
                                      data = subset(data_leg_hybrid, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
...
# Comparing AIC scores to model without hybrid score
droughtday_4year_leg_nohybrid <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                                        random = ~ID + momID, maxiter = 400, 
                                        data = subset(data_leg_hybrid, !is.na(prop_droughtdays_4year)& !is.na(maternal_loss_0isNo)))
summary(droughtday_4year_leg_nohybrid)$aic - summary(droughtday_4year_leg_hybrid)$aic   

# Arm 
droughtday_4year_arm_hybrid <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + anubis_admix + I(anubis_admix^2) + factor(maternal_loss_0isNo), 
                                      random = ~ID + momID, maxiter = 400, 
                                      data = subset(data_forearm_hybrid, !is.na(prop_droughtdays_4year)& !is.na(maternal_loss_0isNo)))
...
# Comparing AIC scores to model without hybrid score
droughtday_4year_arm_nohybrid <- asreml(fixed = log(forearm_cm) ~ log(age) + I(log(age)^2) + mom_proprank_birth + prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                                        random = ~ID + momID, maxiter = 400, data = subset(data_forearm_hybrid, !is.na(prop_droughtdays_4year)))
...
hybrid_p$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
hybrid_beta$body_part <- c("SR", "SR", "SR", "SR", "SR", "SR", "SR", "SR", "SR", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Leg", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm", "Forearm")
...
# Testing interactions btw drought, maternal loss, and maternal rank  ----------------------------------------------------
# Three interactions: drought*maternal loss, drought*maternal rank, and maternal loss*maternal rank.
# Testing in the model in which we found strongest effect of drought: Leg ~ prop drought days in 4 yrs + materna loss

# drought*maternal loss
drought_matloss <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth + prop_droughtdays_4year*factor(maternal_loss_0isNo), 
                          random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                          data = subset(leg, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
wald(drought_matloss)
summary(drought_matloss)$varcomp

# drought*maternal rank
drought_momrank <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth*prop_droughtdays_4year + factor(maternal_loss_0isNo), 
                          random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                          data = subset(leg, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
wald(drought_momrank)
summary(drought_momrank)$varcomp


# maternal loss*maternal rank
matloss_momrank <- asreml(fixed = log(leg_cm) ~ log(age) + I(log(age)^2) + leg_rating + mom_proprank_birth*factor(maternal_loss_0isNo) + prop_droughtdays_4year, 
                          random = ~ID + vm(ID, inverse_pedigree) + momID, maxiter = 400, 
                          data = subset(leg, !is.na(prop_droughtdays_4year) & !is.na(maternal_loss_0isNo)))
wald(matloss_momrank)
summary(matloss_momrank)$varcomp


# Figure 3 - plotting raw data & life history milestones --------------------------------------------------------

# Using median menarche and live birth for the  study subjects. Menarche = 4.7, live birth = 6.3
# Note that I might end up switching this to the values reported in Onyango 2016: Menarche = 4.5, live birth = 6.0 - but this code will be published before the manuscript.

## Using the line of best fit from the 4 year drought + maternal loss (with pedigree) model, 
# with 0.787 for proportion of drought days (which was the mean for our study subjects), no maternal loss (0), and a mid-ranking mom (0.5)
plotlog_sr <- function(x) exp(3.105 + (log(x)*0.614) + (log(x)^2*(-0.117)) + .787*-0.049 + 0.5*-0.001)
plotlog_leg <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + .787*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)
plotlog_forearm <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + .787*-0.357 + 0.5*0.026) 

srplot <- sr %>% # Processing the raw data into a mean value for each baboon
  group_by(ID) %>% 
  summarise(sr_width = mean(sr_cm), mean_age = mean(age), sr_sd = sd(sr_cm), min_age = min(age), max_age = max(age)) %>%
  full_join(., sr, by = "ID") %>%
  ...
  geom_vline(xintercept = 6.3, color = "#E0BF16", size = 1.5) +
  labs(x='', y='Shoulder-Rump (cm)') +
  geom_point(aes(y=sr_width), size = 2.5) +
  geom_point(aes(y=sr_cm), size = .5, alpha = 0.15) +
  stat_function(fun = plotlog_sr, size = 1.5)

legplot <- leg %>% # Processing the raw data into a mean value for each baboon
  group_by(ID) %>% 
  summarise(mean_leg = mean(leg_cm), mean_age = mean(age), leg_sd = sd(leg_cm), min_age = min(age), max_age = max(age)) %>%
  full_join(., leg, by = "ID") %>%
  ggplot(., aes(x=mean_age)) +
  theme(axis.text = element_text(color = "black", size = 16), axis.title = element_text(size = 22), panel.grid.minor = element_blank(), panel.grid.major = element_line(color = "lightgrey"), axis.ticks = element_blank(),
        panel.background = element_rect(fill = "white", colour = "white"))+
  scale_x_continuous(breaks = seq(4,22,2)) + scale_y_continuous(breaks = seq(18,52,2), limits = c(33, 49)) +
  geom_vline(xintercept = 4.7, color = "#199494", size = 1.5) +
  geom_vline(xintercept = 6.3, color = "#E0BF16", size = 1.5) +
  labs(x='', y='Leg (cm)') +
  geom_point(aes(y=mean_leg), size = 2.5) +
  geom_point(aes(y=leg_cm), size = .5, alpha = 0.15) +
  stat_function(fun = plotlog_leg, size = 1.5)

armplot <- forearm %>% # Processing the raw data into a mean value for each baboon
  group_by(ID) %>% 
  summarise(forearm = mean(forearm_cm), mean_age = mean(age), forearm_sd = sd(forearm_cm), min_age = min(age), max_age = max(age)) %>%
  full_join(., forearm, by = "ID") %>%
  ggplot(., aes(x=mean_age)) +
  theme(axis.text = element_text(color = "black", size = 16), axis.title = element_text(size = 22), panel.grid.minor = element_blank(), panel.grid.major = element_line(color = "lightgrey"), axis.ticks = element_blank(),
        panel.background = element_rect(fill = "white", colour = "white"))+
  scale_x_continuous(breaks = seq(4,22,2)) + scale_y_continuous(breaks = seq(18,52,2)) +
  ...
  geom_vline(xintercept = 6.3, color = "#E0BF16", size = 1.5) +
  labs(x='Age (years)', y='Forearm (cm)') +
  geom_point(aes(y=forearm), size = 2.5) +
  geom_point(aes(y=forearm_cm), size = .5, alpha = 0.15) +
  stat_function(fun = plotlog_forearm, size = 1.5)
library(gridExtra)
grid.arrange(srplot, legplot, armplot, ncol = 1)
g <- arrangeGrob(srplot, legplot, armplot, ncol = 1)
ggsave(file = "Figure 3.eps", width = 6, height = 12, units = "in", dpi = 600, g)
ggsave(file = "Figure 3.pdf", width = 6, height = 12, units = "in", dpi = 600, g)



# Figure 4 - model effects ------------------------------------
## Using the model outcomes from the 4 year drought + maternal loss (with pedigree) models (leg and forearm), 
# Varying the proportion of drought days to make different predicted lines...
# with 0.787 = mean. 
# For other predictors: line codes for no maternal loss (0), and a mid-ranking mom (0.5)

plotlog_legmax <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + .882*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)
plotlog_leg1sdhigh <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + .823*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)
plotlog_legmean <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + .787*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)
plotlog_leg1sdlow <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + 0.751*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)
plotlog_legmin <- function(x) exp(3.221 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 1.72*0.017 + 0.691*-0.307 + 0.5*0.019) # Leg rating of 1.72 (mean leg rating for this dataset)

plotlog_forearm_max <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + .882*-0.357 + 0.5*0.026) 
plotlog_forearm_1sdhigh <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + .823*-0.357 + 0.5*0.026) 
plotlog_forearm_mean <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + .787*-0.357 + 0.5*0.026) 
plotlog_forearm_1sdlow <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 0.751*-0.357 + 0.5*0.026) 
plotlog_forearm_min <- function(x) exp(2.670 + (log(x)*0.601) + (log(x)^2*(-0.119)) + 0.691*-0.357 + 0.5*0.026) 

legplot2 <- leg %>% # Processing the raw data into a mean value for each baboon
  group_by(ID) %>% 
  summarise(mean_leg = mean(leg_cm), mean_age = mean(age), leg_sd = sd(leg_cm), min_age = min(age), max_age = max(age)) %>%
  full_join(., leg, by = "ID") %>%
  ggplot(., aes(x=mean_age)) +
  ...
  scale_x_continuous(breaks = seq(4,22,2)) + scale_y_continuous(breaks = seq(18,52,2), limits = c(33, 48)) +
  labs(x='', y='Leg (cm)') +
  geom_point(aes(y=mean_leg), size = 2.5, color = "grey70") +
  stat_function(fun = plotlog_legmax, size = 1.5, color = "#AE0000") +
  stat_function(fun = plotlog_leg1sdhigh, size = 1.5, color = "#E33E00") +  
  stat_function(fun = plotlog_legmean, size = 1.5, color = "#FFC000") +  
  stat_function(fun = plotlog_leg1sdlow, size = 1.5, color = "#0088AB") +
  stat_function(fun = plotlog_legmin, size = 1.5, color = "#03286B") # These colors are from MetBrewer - Jitterbugs by William Henry Johnson. 



armplot2 <- forearm %>% # Processing the raw data into a mean value for each baboon
  group_by(ID) %>% 
  summarise(forearm = mean(forearm_cm), mean_age = mean(age), forearm_sd = sd(forearm_cm), min_age = min(age), max_age = max(age)) %>%
  full_join(., forearm, by = "ID") %>%
  ggplot(., aes(x=mean_age)) +
  ...
  scale_x_continuous(breaks = seq(4,22,2)) + scale_y_continuous(breaks = seq(18,52,2)) +
  labs(x='Age (years)', y='Forearm (cm)') +
  geom_point(aes(y=forearm), size = 2.5, color = "grey70") +
  stat_function(fun = plotlog_forearm_max, size = 1.5, color = "#AE0000") +
  stat_function(fun = plotlog_forearm_1sdhigh, size = 1.5, color = "#E33E00") +  
  stat_function(fun = plotlog_forearm_mean, size = 1.5, color = "#FFC000") +  
  stat_function(fun = plotlog_forearm_1sdlow, size = 1.5, color = "#0088AB") +
  stat_function(fun = plotlog_forearm_min, size = 1.5, color = "#03286B") # These colors are from MetBrewer - Jitterbugs by William Henry Johnson. 

grid.arrange(legplot2, armplot2, ncol = 1)
g <- arrangeGrob(legplot2, armplot2, ncol = 1)

ggsave(filename = "Figure 4.eps", width = 6, height = 8, units = "in", dpi = 600, g)
ggsave(filename = "Figure 4.pdf", width = 6, height = 8, units = "in", dpi = 600, g)


# Figure S4 - data with error bars ---------------------------------------------------------------
...
random_effects <- data.frame(c("Shoulder-Rump","Shoulder-Rump","Shoulder-Rump","Shoulder-Rump","Leg","Leg","Leg","Leg","Forearm","Forearm","Forearm","Forearm"),
                             ...
                             c(0.58, 0.0000002610, 0.0000000558, 0.42,0.67, 0.13, 0.000182, 0.22,0.41, 0.13, 0.0000000239, 0.46))
...
ggsave("Heritability figure.pdf", width = 6, height = 4, units = "in", dpi = 600)



# Figure S2 - Importance of sample size -----------------------------------------------
